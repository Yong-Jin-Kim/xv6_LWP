...
extern int sys_print_order(void);
...
[SYS_print_order] sys_print_order,
};
...
syscall(void)
{
  ...
  if(curproc->proc_true == 1) {
    ...
    num = curproc->t_tf[curproc->active_thread]->eax;
    if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
      curproc->t_tf[curproc->active_thread]->eax = syscalls[num]();
    } else {
      cprintf("%d %s: unknown sys call %d\n",
	  curproc->pid, curproc->name, num);
      curproc->t_tf[curproc->active_thread]->eax = -1;
    }
  }
}
